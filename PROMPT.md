...
应用应该采用移动端优先的设计
    ...
    - 用户点击顶部导航栏目，进入导航页
    - 导航页提供导航宫格供用户浏览，用户点击导航宫格，进入列表页
通常的，列表页有一级入口；用户在列表页的一个条目上点击，进入表单页.
...
Directus 数据模型的集合之间的实体关系，也有对应的表单控件来表达
    ...
    - Dropdown 类型的字段，少于 5 个选项时，用 Radio.Group 控件；否则用 Select 控件
    - 文本类型的字段用 Input 控件
    - 多行文本类型的字段用 Input.TextArea 控件
    - 如果字段不允许修改，则直接用 curly braces 包裹内容

## Ant Design 表单控件 Form 和 Form.Item 的使用要点
表单使用 Form1 这个自定义组件.
表单有 FormAction 这个自定义组件，内部至少含有一个 primary Button 提交保存数据.
Form.Item 放在 `<div className="form-grid">` 内部，以满足响应式布局.
Form.Item 具有 `className="form-item"` label 和 name 三个属性
    - 根据 antd 文档，被设置了 name 属性的 Form.Item 包装的控件，数据同步将被 Form 接管；
    - 提交时，onFinish 事件仅仅收集这些数据

## 用户、部门与成员
如果应用涉及到部门与成员，则按照下列概念来设计 Directus 数据模型.
...
## 产品与订单
如果应用涉及到产品与订单，则按照下列概念来设计 Directus 数据模型.
"产品 (Product)"，是交易物.
"订单 (Order)"，是交易记录.
